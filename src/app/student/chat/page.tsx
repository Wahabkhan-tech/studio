...
const initialMessages: {
  ...
  [myGroup.id]: [
    {
      id: 'm1',
      senderId: 'EB22210006139',
      text: "Hey, I've pushed the initial schema design. Can you take a look?",
      timestamp: '09:30 AM',
    },
    {
      id: 'm2',
      senderId: 'EB22210006137',
      text: 'Sure, checking it out now. Looks like a good start!',
      timestamp: '09:32 AM',
    },
    {
      id: 'm3',
      senderId: 'EB22210006139',
      text: 'Great. Let me know if any changes are needed for the auth part.',
      timestamp: '09:33 AM',
    },
  ],
  [supervisor.id]: [
    {
      ...
      text: `Hello ${supervisor.name}, I have a question about the project proposal.`,
      timestamp: 'Yesterday',
    },
  ],
};
...
export default function StudentChatPage() {
  ...
  const handleCreateChat = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const memberIds = students
      .filter((s) => formData.get(s.id))
      .map((s) => s.id);

    if (!chatName.trim() || memberIds.length === 0) {
      toast({
        title: 'Error',
        description: 'Chat name and at least one member are required.',
        variant: 'destructive',
      });
      return;
    }
    ...
    toast({
      title: 'Chat Created!',
      description: `You started the "${chatName}" chat.`,
    });
  };
  ...
  const handleAddMembers = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    toast({
      title: 'Members Added',
      description: `${newMemberIds.length} new member(s) have been added to the chat.`,
    });
  };
  ...
  const otherStudentsInSystem = students.filter(
    (s) => s.id !== loggedInStudent.id
  );
  const membersInCurrentCustomChat = selectedConversation?.members || [];
  const studentsNotInCurrentChat = otherStudentsInSystem.filter(
    (s) => !membersInCurrentCustomChat.includes(s.id)
  );
          ...
          <Dialog open={isCreateChatOpen} onOpenChange={setIsCreateChatOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a temporary chat with other students.
                </DialogDescription>
              </DialogHeader>
                        ...
                        >
                          ...
                          >
                            <Avatar className="h-8 w-8">
                              <AvatarImage
                                src={getAvatarUrl(student.id, 'student')}
                              />
                              <AvatarFallback>
                ...
                <Dialog open={isAddMemberOpen} onOpenChange={setIsAddMemberOpen}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      ...
                      <DropdownMenuItem
                        ...
                        onClick={() => handleLeaveChat(selectedConversationId)}
                      >
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Select students to add to this chat.
                      </DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleAddMembers}>
                      <div className="py-4">
                        <ScrollArea className="h-60 rounded-md border p-4">
                          {studentsNotInCurrentChat.length > 0 ? (
                            studentsNotInCurrentChat.map((student) => (
                              <div
                                key={student.id}
                                className="flex items-center gap-3 mb-2"
                              >
                            ...
                            <p className="text-sm text-muted-foreground text-center">
                              All students are already in this chat.
                            </p>
                          )}
                        </ScrollArea>
                      </div>
            ...
            <ScrollArea className="flex-1 p-4">
              <div className="space-y-4">
                {currentMessages.map((message) => {
                  const convMembers = students.filter((s) =>
                    selectedConversation.members?.includes(s.id)
                  );
                  const sender =
                    convMembers.find((m) => m.id === message.senderId) ||
                    (message.senderId === supervisor.id
                      ? supervisor
                      : loggedInStudent);
